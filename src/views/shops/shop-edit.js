...
const ShopsEdit = () => {
  ...
  function getLanguageFields(data) {
    ...
    const result = languages.map((item) => ({
      [`title[${item.locale}]`]: translations.find((el) => el?.locale === item.locale)?.title,
      [`description[${item.locale}]`]: translations.find((el) => el?.locale === item.locale)?.description,
      [`address[${item.locale}]`]: translations.find((el) => el?.locale === item.locale)?.address,
    }));
