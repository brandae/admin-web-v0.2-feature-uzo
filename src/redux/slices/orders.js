...
const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchNewOrders.fulfilled, (state, action) => {
      ...
      const currentOrders = [ ...state.items.new ]
      const current = currentOrders.map( o => o.id )
      const newOrders = payload.orders.filter( o => !current.includes(o.id))

      if(newOrders.length + currentOrders.length > 5) {
        currentOrders.pop()
      }
